...
// syllomatter.cpp : Defines the entry point for the console application.
//
...
static svn_error_t * my_ssl_server_trust_prompt_callback(svn_auth_cred_ssl_server_trust_t **cred,void *baton,const char *realm,apr_uint32_t failures,const svn_auth_ssl_server_cert_info_t *cert_info,svn_boolean_t may_save,apr_pool_t *pool)
{
   *cred = (svn_auth_cred_ssl_server_trust_t *)apr_palloc(pool,sizeof(svn_auth_cred_ssl_server_trust_t));
   (*cred)->may_save = true;
...
int libsvn_minimal_client_test(::syllomatter::list_view * plist, const char * strUrl)
{
   ...
   if(svn_cmdline_init("syllomatter.exe",stderr) != EXIT_SUCCESS)
      return EXIT_FAILURE;
   ...
   /* All clients need to fill out a client_ctx object. */
  {
     ...
     /* Make the client_ctx capable of authenticating users */
    {
       ...
       svn_auth_get_ssl_server_trust_prompt_provider(&provider,
          my_ssl_server_trust_prompt_callback,
          NULL,
          pool);
       APR_ARRAY_PUSH(providers,svn_auth_provider_object_t *) = provider;
